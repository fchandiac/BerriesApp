...
function CustomPagination(props) {
    ...
    let balanceTotal = 0
    rows.map(row => {
        balanceTotal = balanceTotal + row[1].balance
    })
    ...
    const exportExcel = () => {
        ...
        sheet.addRow([])
        sheet.addRow(['','','','','TOTAL SALDO', balanceTotal])
    ...
    return (
        <Box sx={{ p: 1 }}>
            ...
            >
                ...
                />
                <Typography>Total saldo: {utils.renderMoneystr(balanceTotal)}</Typography>
            </Stack>
        </Box>
...
export default function PaysGridBase(props) {
